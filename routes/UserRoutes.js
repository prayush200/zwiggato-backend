...
const jwtSecret="Mynameisfullstackwebdevelopermernstack"
...
body('password','incorrect password').isLength({ min: 5 }),async(req,res)=>{
    ...
    try {
      await User.create({
            name:req.body.name,
            password:secPassword, //ENCODING PASSWORD
...
body('password','incorrect password').isLength({ min: 5 }), async(req,res)=>{  
  ...
  let email =req.body.email;
    try {
      ...
      const pwdCompare= await bcrypt.compare(req.body.password , userData.password  )      //Comparing orignal pass with encoded password
      if(!pwdCompare){
        return res.status(400).json({error:"Try logging with correct credentials"})
      }
